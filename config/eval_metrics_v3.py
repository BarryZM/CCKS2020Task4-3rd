...
def error_output_csv_2(file_pre, file_true, output_dir):
    ...
    pre_dict = dict()
    true_dict = dict()
    for line in open(file_pre,'r', encoding="utf-8"):
        line = line.strip()
        line = json.loads(line)
        ids = line['id']
        pre_dict[ids] = line['events']
        content_dict[ids] = line['content']
    for line in open(file_true,'r', encoding="utf-8"):
        line = line.strip()
        line = json.loads(line)
        ids = line['id']
...
def evaluate(file_pre, file_true):
    ...
    for type in type_dict.keys():
        ...
        print('*'*50)
        print("%s\tPrecision\tRecall\tF1"%(type))
        ...
        for i in range(len(type_dict[type])):
            ...
            recall = p * 1.0 / total_entity * 100 if total_entity != 0 else 0
            fscore = 2.0 * precision * recall / (precision + recall) if precision != 0 or recall != 0 else 0
            print("%s\t%.2f\t%.2f\t%.2f" % (type_dict[type][i], precision, recall, fscore))
        print("%s\t%.2f\t%.2f\t%.2f" % ("total", total_precision, total_recall, total_fscore))
...
if __name__ == "__main__":
    file_true = 'D:/allprojects/bisai/ccks_3/train/test_right_datas.json'
    file_pre = 'D:/allprojects/bisai/ccks_3/train_valid_result.json'
    # file_pre = 'D:/allprojects/bisai/ccks_3/revise/train_revise_valid_result.json'
    output_dir = 'D:/allprojects/bisai/ccks_3/error/error_info_0.csv'
